...
git config --global user.email "あなたのメール@goodpatch.co.jp"
```
...
allowed_email_domains = ["goodpatch.co.jp", "anywhere.goodpatch.com"]
```
...
ログインは @goodpatch.co.jp のメールアドレスが必要です。
...
### ❌ ログインできない
→ secrets.toml の `allowed_email_domains` を確認。デフォルトは `goodpatch.co.jp` と `anywhere.goodpatch.com`
